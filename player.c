...
void load_announce_file() {
	...
	if(len > 1024*1024*1024) { // don't allocate huge buffer in case it's broken
		announce_data = 0;
		announce_file = 0;
		announce_len = 0;
		return;
...
int load_lavc( char* file_name ) {
	...
	do {
		...
		while( bytes_remaining > 0 ) {
			...
			int i;
			for(i = 0; announce_pos_c < announce_len && i < 8192; i++ ) {
				announce_pos_c++;
				decoded_data[i] = (announce_data[announce_pos_c]) / 2;
			}
