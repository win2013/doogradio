...
def send(buf):
    ...
    s.send(buf)
    #time.sleep( 0.070 + random.random() * 0.1 ) # network simulator
    packets_sent += 1
    ...
    print "sleeping for %f, %d packets sent, %d packets needed" % ((total_packet_time - packet_elapsed_time), packets_sent, packets_needed)
    if( total_packet_time - packet_elapsed_time < 0):
    	time.sleep(0)
    else:
